...
fun testGraphCreation() {
    ...
    fun <S> GraphTraversal<S, Vertex>.traversalTest1(): GraphTraversal<S, Vertex> = this.hasLabel("mftType").out("senderComponent")
    ...
    val postscriptL3X: Vertex =
        g.addV("postScript").property("name", "mv var/tmp && op asipost.sh; mv var/tmp/11 && op asipost22.sh").next()
    ...
    g.addE("hasParams").from(postscriptL3X).to(postscriptParams_CH_nodesan).to(postscriptParams_OTHR_desan).iterate()
    g.addE("hasReceiverDirectory").from(postscriptL3X).to(receiverDirectoryL3X).iterate()

    //fun <S, R>GraphTraversal<S, R>.outOptional(vararg edgeLabels:String):GraphTraversal<S, Vertex> =
    //    this.coalesce(this.out(*edgeLabels), g.V(notAvailable.id()))

    //mftType->hasPostScript, receiverComponent->hasPostScript

    fun <S> GraphTraversal<S, Vertex>.componentToServerGroupTraversal(party: String): GraphTraversal<S, Vertex> =
        this.select<Vertex>("${party}Component").match<Vertex>(
            As<S>("${party}Component").coalesce(out("deployedOn"), g.V(notAvailable.id()))
                .`as`("${party}ServerGroup"),
            As<S>("${party}ServerGroup").coalesce(out("restrictedServesManEnv"), g.V(notAvailable.id()))
                .`as`("${party}RestrictedServesManEnv"),
            As<S>("${party}RestrictedServesManEnv").or(
                where<S>(P.eq("${party}ManEnvAppl")),
                hasLabel<S>("notAvailable")
            )
        ).select<Vertex>("${party}ServerGroup").`as`("${party}ServerGroup")

    fun <S> GraphTraversal<Vertex, S>.componentToServerGroupTraversalInv(party: String): GraphTraversal<Vertex, S> =
        this.select<S>("${party}Component").match<S>(
            As<S>("${party}Component").coalesce(out("deployedOn"), g.V(notAvailable.id()))
                .`as`("${party}ServerGroup"),
            As<S>("${party}ServerGroup").coalesce(out("restrictedServesManEnv"), g.V(notAvailable.id()))
                .`as`("${party}RestrictedServesManEnv"),
        ...
        ).select<S>("${party}ServerGroup").`as`("${party}ServerGroup")
    ...
    g.V().match<String>(
        ...
        //.select<String>("receiverComponent")
        //.componentToServerGroupTraversalInv<String>("receiver")

        .select<String>("receiverComponent")
        .match<String>(
            As<String>("receiverComponent").coalesce(out("deployedOn"), g.V(notAvailable.id()))
                .`as`("receiverServerGroup"),
            As<String>("receiverServerGroup").coalesce(out("restrictedServesManEnv"), g.V(notAvailable.id()))
                .`as`("receiverRestrictedServesManEnv"),
            As<String>("receiverRestrictedServesManEnv").or(
                where<String>(P.eq("receiverManEnvAppl")),
                hasLabel<String>("notAvailable")
            )
        )
        .select<String>("receiverServerGroup").`as`("receiverServerGroup")
        .select<String>("senderComponent").match<String>(
            As<String>("senderComponent").coalesce(out("deployedOn"), g.V(notAvailable.id())).`as`("senderServerGroup"),
            As<String>("senderServerGroup").coalesce(out("restrictedServesManEnv"), g.V(notAvailable.id()))
                .`as`("senderRestrictedServesManEnv"),
            As<String>("senderRestrictedServesManEnv").or(
                where<String>(P.eq("senderManEnvAppl")),
                hasLabel<String>("notAvailable")
            )
